...
final class SearchBar: UITextField {
    ...
    private func bindUI() {
        ...
        Observable.combineLatest(self.rx.text.orEmpty, searchResultExists)
            .map { text, resultExists -> CGColor in
                if text.isEmpty {
                    return UIColor(hexString: "D2CEC6").cgColor
                } else {
                    return resultExists ? UIColor(hexString: "29BF6A").cgColor : UIColor(hexString: "FF3B30").cgColor
                }
            }
            .bind(onNext: { [weak self] borderColor in
